...
Feature: Edit an external assignment to add due/cut off dates without completion conditions
  In order to manage assignments effectively
  As a teacher
  I need to be able to edit an existing external assignment to add specific dates without
  adding any completion conditions

  This covers acceptance_tests.md section 3 ("Edit existing external assignment"), row c.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | externalassignment | C1     | Assignment without dates | m999-editcutoffnone | https://www.example.com/assignment | 0          |

  Scenario: Editing an external assignment to add due/cut off dates without completion conditions
